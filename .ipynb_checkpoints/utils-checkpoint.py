...
def image2patch(images, patch_size = 80):
    if len(images.shape) == 4:
        return einops.rearrange(images, 'b (h p1) (w p2) c -> (b h w) p1 p2 c',
                        p1 = patch_size,
                        p2 = patch_size)
    return einops.rearrange(images, 'b (h p1) (w p2) -> (b h w) p1 p2',
                    p1 = patch_size,
                    p2 = patch_size)

def patch2image(images, num_patches = 1, patch_size = 80):
    if len(images.shape) == 4:
        return einops.rearrange(images, '(b n m) p1 p2 c -> b (n p1) (m p2) c',
                    p1 = patch_size,
                    p2 = patch_size,
                    n = num_patches,
                    m = num_patches)
    return einops.rearrange(images, '(b n m) p1 p2 -> b (n p1) (m p2)',
                    p1 = patch_size,
                    p2 = patch_size,
                    n = num_patches,
                    m = num_patches)
...
def AUC_ROC(true_vessel_arr, pred_vessel_arr, save_fname):
    ...
    """
    #print("true vessel {},pred {}".format(np.unique(true_vessel_arr),np.unique(pred_vessel_arr)))
...
def plot_AUC_ROC(fprs, tprs, method_names, fig_dir, op_pts):
    ...
    colors = ['r', 'b', 'y', 'g', '#7e7e7e', 'm', 'c', 'k', '#cd919e', "#691402"] if len(fprs) == 10 else ['r', 'y', 'm', 'g', 'k', "b"]
    indices = [7, 2, 5, 3, 9, 4, 6, 1, 8, 0] if len(fprs) == 10 else [4, 1, 2, 3, 5, 0]  

    # print auc  
    print("****** ROC AUC ******")
    print("CAVEAT : AUC of RetinaGAN with 8bit images might be lower than the floating point array (check <home>/pretrained/auc_roc*.npy)")
    for index in indices:
        if method_names[index] != 'CRFs' and method_names[index] != '2nd_manual' and method_names[index] != 'FC-CRF':
    ...
    for index in indices:
        ...
        elif method_names[index] == 'FC-CRF':
            plt.plot(fprs[index], tprs[index], colors[index] + '*', label='FC-CRF')
        else:
    ...
    if len(fprs) == 10:
        plt.title('ROC Curve (DRIVE)')
    else:
        plt.title('ROC Curve (STARE)')
...
def plot_AUC_PR(precisions, recalls, method_names, fig_dir, op_pts):
    ...
    # sort the order of plots manually for eye-pleasing plots
    colors = ['r', 'b', 'y', 'g', '#7e7e7e', 'm', 'c', 'k', '#cd919e', "#691402"] if len(precisions) == 10 else ['r', 'y', 'm', 'g', 'k', "b"]
    indices = [7, 2, 5, 3, 9, 4, 6, 1, 8, 0] if len(precisions) == 10 else [4, 1, 2, 3, 5, 0] 

    # print auc  
    print ("****** Precision Recall AUC ******")
    print ("CAVEAT : AUC of RetinaGAN with 8bit images might be lower than the floating point array (check <home>/pretrained/auc_pr*.npy)")
    for index in indices:
        if method_names[index] != 'CRFs' and method_names[index] != '2nd_manual' and method_names[index] != 'FC-CRF':
    ...
    for index in indices:
        if method_names[index] == 'CRFs':
            ...
            plt.plot(recalls[index], precisions[index], colors[index] + '*', label='FC-CRF')
        else:
    ...
    if len(precisions) == 10:
        plt.title('Precision Recall Curve (DRIVE)')
    else:
        plt.title('Precision Recall Curve (STARE)')
    plt.xlabel("Recall")
...
def get_imgs(target_dir, augmentation, img_size, dataset, mask=False):
    ...
    if augmentation:
        ...
        fundus_imgs = np.concatenate(all_fundus_imgs, axis=0, dtype = np.float32)
        vessel_imgs = np.asarray(np.round((np.concatenate(all_vessel_imgs, axis=0))), dtype = int)
    
    # z score with mean, std of each image
    n_all_imgs = fundus_imgs.shape[0]
    for index in range(n_all_imgs):
        mean = np.mean(fundus_imgs[index, ...][fundus_imgs[index, ..., 0] > 40.0], axis=0, dtype = np.float32)
        std = np.std(fundus_imgs[index, ...][fundus_imgs[index, ..., 0] > 40.0], axis=0, dtype = np.float32)
